...
<script setup lang="ts">
	...
	const platformOptions = computed(() => {
		const output: Map<Platform<AMMO_TYPE>, { label: string; searchKeys: string[]; icon?: any }> = new Map();

		for (const ammoType of Object.keys(ARTILLERY_BY_SHELL) as AMMO_TYPE[]) {
			if (props.ammoType != null && ammoType !== props.ammoType) continue;

			for (const platform of Object.keys(ARTILLERY_BY_SHELL[ammoType].PLATFORM) as Platform<AMMO_TYPE>[]) {
				output.set(platform, { label: platform, searchKeys: [ammoType], icon: markRaw(ICONS[platform]) });
			}
		}
