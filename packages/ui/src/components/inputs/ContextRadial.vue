...
<script setup lang="ts">
	...
	import { getAvailableUnitTypes } from '@/lib/unit';

	export type Payload = Pick<Unit, 'type' | 'ammunition' | 'platform' | 'spottingType'>;
	...
	const options: Options<Payload> = new Map();
	...
	for (const [index, unitType] of getAvailableUnitTypes().entries()) {
		...
		if (unitType === UnitType.Artillery) {
			...
			for (const [index, ammoType] of (
				...
				unitTypeSubOptions.set({ type: unitType, ammunition: ammoType }, ammoTypeOption);
			}
		...
		if (unitType === UnitType.Spotter) {
			...
			for (const [index, spottingType] of Object.values(SPOTTING_TYPE)
				...
				unitTypeSubOptions.set({ type: unitType, spottingType }, spottingTypeOption);
			}

			unitTypeSubOptions.set({ type: unitType }, {
				label: 'None',
				icon: UNIT_ICON_BY_TYPE[unitType],
				order: 0,
			});
		...
		options.set({ type: unitType }, unitTypeOption);
	}

	new Map(
		getAvailableUnitTypes().map((unitType, index) => [
			unitType,
			{
				label: UnitType[unitType],
				icon: UNIT_ICON_BY_TYPE[unitType],
